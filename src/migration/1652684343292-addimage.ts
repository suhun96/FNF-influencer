...
export class addimage1652684343292 implements MigrationInterface {
    name = 'addimage1652684343292'
    ...
    public async up(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`CREATE TABLE \`image\` (\`id\` int NOT NULL AUTO_INCREMENT, \`image_url\` varchar(255) NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        await queryRunner.query(`CREATE TABLE \`influencer_image\` (\`id\` int NOT NULL AUTO_INCREMENT, \`influencerID\` int NOT NULL, \`imageID\` int NOT NULL, \`created_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6), \`updated_at\` datetime(6) NOT NULL DEFAULT CURRENT_TIMESTAMP(6) ON UPDATE CURRENT_TIMESTAMP(6), PRIMARY KEY (\`id\`)) ENGINE=InnoDB`);
        ...
        await queryRunner.query(`ALTER TABLE \`influencer_image\` ADD CONSTRAINT \`FK_f347b8317dd5053510def3fbbda\` FOREIGN KEY (\`influencerID\`) REFERENCES \`influencer\`(\`id\`) ON DELETE NO ACTION ON UPDATE NO ACTION`);
        await queryRunner.query(`ALTER TABLE \`influencer_image\` ADD CONSTRAINT \`FK_588e7eb221250ec980835e4a143\` FOREIGN KEY (\`imageID\`) REFERENCES \`image\`(\`id\`) ON DELETE CASCADE ON UPDATE NO ACTION`);
    ...
    public async down(queryRunner: QueryRunner): Promise<void> {
        ...
        await queryRunner.query(`ALTER TABLE \`influencer_image\` DROP FOREIGN KEY \`FK_588e7eb221250ec980835e4a143\``);
        await queryRunner.query(`ALTER TABLE \`influencer_image\` DROP FOREIGN KEY \`FK_f347b8317dd5053510def3fbbda\``);
        ...
        await queryRunner.query(`DROP TABLE \`influencer_image\``);
